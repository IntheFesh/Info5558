...
CONFIG = {
    ...
    "LightGBM_epoch": 4163,             # LightGBM 固定 boosting 轮数
    "XGBoost_epoch": 3187,              # XGBoost 固定 boosting 轮数
    "TextModel_epoch": 32,              # TextModel / AdapterRegressor 固定训练 epoch 数
    "TabM_epoch": 32,                   # TabM 固定训练 epoch 数

    # 路径与基础设置
    "DATA_DIR": r"C:\Users\moqia\Documents\xwechat_files\wxid_gncgtvzfzbzg22_6451\msg\file\2026-04\app-of-gen-ai-deep-learning-wustl-spring-2026(1)",                    # 训练/测试 CSV 所在目录（默认当前目录）
    "OUTPUT_DIR": r"C:\Users\moqia\Documents\xwechat_files\wxid_gncgtvzfzbzg22_6451\msg\file\2026-04\app-of-gen-ai-deep-learning-wustl-spring-2026(1)\result",            # 提交结果输出目录
    "N_FOLDS": 5,                       # 交叉验证折数
    ...
    # CatBoost
    "CATBOOST_LEARNING_RATE": 0.018133600141977357,    # 学习率（参考过去的数值：0.018，LR 0.015 + 5200 epoch 更差）
    "CATBOOST_DEPTH": 5,                # 树深度
    "CATBOOST_L2_LEAF_REG": 7.839958962696954,       # L2 叶节点正则系数
    "CATBOOST_SUBSAMPLE": 0.908744269754171,         # 行采样比例
    "CATBOOST_RSM": 0.6159118714654215,                # 列采样比例
    "CATBOOST_RANDOM_STRENGTH": 1.4836390251434222,    # 分裂点选择随机强度
    "CATBOOST_BAGGING_TEMPERATURE": 1.9789225110642295, # Bayesian bootstrap 温度
    ...
    "LIGHTGBM_LEARNING_RATE": 0.03994470251623529,    # 学习率
    "LIGHTGBM_NUM_LEAVES": 97,          # 单棵树最大叶子数（47 偏保守，恢复 63 保证容量）
    "LIGHTGBM_MIN_CHILD_SAMPLES": 14,   # 叶节点最少样本数
    "LIGHTGBM_MAX_DEPTH": 3,           # 不限最大深度，由 num_leaves 控制树形
    "LIGHTGBM_SUBSAMPLE": 0.683261723963235,         # 行采样比例
    "LIGHTGBM_COLSAMPLE": 0.8096138269318928,          # 列采样比例
    "LIGHTGBM_L1": 0.002907822686447079,                # L1 正则
    "LIGHTGBM_L2": 0.009242186837720087,                 # L2 正则

    # XGBoost
    "XGBOOST_LEARNING_RATE": 0.021965215011981985,      # 学习率
    "XGBOOST_MAX_DEPTH": 3,             # 树深度
    "XGBOOST_MIN_CHILD_WEIGHT": 7.691815583139432,      # 叶节点最小 hessian 和（加大，避免过拟合小叶）
    "XGBOOST_SUBSAMPLE": 0.7062908121964223,           # 行采样比例
    "XGBOOST_COLSAMPLE": 0.6665147392828112,           # 列采样比例
    "XGBOOST_REG_ALPHA": 0.243623966334543,           # L1 正则
    "XGBOOST_REG_LAMBDA": 0.013883874826119628,          # L2 正则
    ...
    for fold, (tr_idx, va_idx) in enumerate(folds, start=1):
        ...
        with suppress_stdout_stderr():
            model = lgb.train(
                params,
                dtr,
                num_boost_round=CONFIG["LightGBM_epoch"],
                valid_sets=[dva],
    ...
    for fold, (tr_idx, va_idx) in enumerate(folds, start=1):
        ...
        with suppress_stdout_stderr():
            model = xgb.train(
                params,
                dtr,
                num_boost_round=CONFIG["XGBoost_epoch"],
                evals=[(dva, "va")],
                verbose_eval=False,
            )
